...
/**
 * Resolves display identity (name/email) for a set of Postgres users, reading
 * from Oracle in a single batch for those mapped to an employee. Users without
 * an Oracle employee use their fallback fields. If Oracle is unavailable, mapped
 * users degrade to their fallback or a generic label so the UI keeps working.
 *
...
export async function resolveUserIdentities(usersInput: UserIdentityInput[]): Promise<Map<string, ResolvedIdentity>> {
  ...
  let employeesById = new Map<number, { name: string; email: string }>();

  if (empIds.length > 0) {
    try {
      const employees = await findEmployeesByIds(empIds);
      employeesById = new Map(Array.from(employees.entries()).map(([empId, employee]) => [empId, { name: employee.name, email: employee.email }]));
    } catch (error) {
