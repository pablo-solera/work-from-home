  ...
  return (
    <section aria-busy="true" className="space-y-8">
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 overflow-hidden"><div className="grid min-w-[80rem] grid-cols-8 gap-4 border-b border-zinc-200 pb-3">{Array.from({ length: 8 }, (_, index) => <Skeleton className="h-4" key={index} />)}</div><div className="min-w-[80rem] divide-y divide-zinc-100">{Array.from({ length: 10 }, (_, index) => <div className="grid grid-cols-8 gap-4 py-5" key={index}>{Array.from({ length: 8 }, (_, cellIndex) => <Skeleton className="h-5" key={cellIndex} />)}</div>)}</div></div>
